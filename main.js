...
class Bank {
    ...
    transaction(obj) {
        let newaccount = this.account.filter((account) => account.accountno !== obj.accountno);
...
for (let i = 1; i < 6; i++) {
    ...
    const cus = new customer(fname, lname, 20 * i, "Male", mobilenumber, 1000 + i);
...
async function Bankservice(Bank) {
    do {
        ...
        if (services.Select == "View balance") {
            ...
            if (account) {
                let name = mybank.Customer.find((acc) => acc.accountnumber == account?.accountno);
                console.log(`Dear${chalk.green.italic(name?.fname)}${chalk.green.italic(name?.lname)} Your account balance is ${chalk.blue.bold(`$${account.balance}`)} `);
            }
        }
        if (services.Select == "Cash withdraw") {
            ...
            if (account) {
                ...
                Bank.transaction({ accountno: account.accountno, balance: newballance });
            }
        }
